...
public class GetSubscriberValidation extends BaseClass{
	...
	public boolean parameter_Validation(String paraString, HashMap<String, String> error) {
		...
		String emailaddress_ismandatory = utils.getproperty("mandatoryfield", "Get_Subscribers_email").trim();
		String mailinglist_ismandatory = utils.getproperty("mandatoryfield", "Get_Subscribers_list").trim();
		...
		try {
			...
			Element ele_searchinfo = ele_details.getChild("searchinfo");
			Element  ele_emailaddress = ele_searchinfo.getChild("Email");
			emailaddress = ele_emailaddress.getText().toString();

			Element  ele_mailinglist = ele_searchinfo.getChild("List");
		...
		if(!isvalidlist(mailinglist, mailinglist_ismandatory, allAvailableLists)) {
			...
			if(errormessage.isEmpty()) {
				if(mailinglist.isEmpty() || mailinglist.trim()=="") {
					errormessage = utils.getproperty("errormessages", "error_missing_searchinfo");
				}
				else {
