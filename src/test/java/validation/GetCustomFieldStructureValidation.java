...
public class GetCustomFieldStructureValidation extends BaseClass{
	...
	public boolean parameter_Validation(String paraString, HashMap<String, String> error) {
		...
		String listId_ismandatory = utils.getproperty("mandatoryfield", "Get_List_Subscribers_listId").trim();
		...
		try {
			...
			Element  ele_listid = ele_details.getChild("listids")
					;
			try {
		...
		if(!isvalidlist(listId, listId_ismandatory, allAvailableLists)) {
			...
			if(errormessage.isEmpty()) {
				if(listId.isEmpty() || listId.trim()=="") {
					errormessage =  utils.getproperty("errormessages", "error_invalid_listids");
				}
				else {
