...
public class UpdateSubscribersValidation extends BaseClass{
	...
	public boolean parameter_Validation(String paraString, HashMap<String, String> error) {
		...
		String emailaddress_ismandatory = utils.getproperty("mandatoryfield", "update_Subscribers_emailaddress").trim();
		String add_to_autoresponders_ismandatory = utils.getproperty("mandatoryfield", "update_Subscribers_add_to_autoresponders").trim();
		String mailinglist_ismandatory = utils.getproperty("mandatoryfield", "update_Subscribers_mailinglist").trim();
		String format_ismandatory = utils.getproperty("mandatoryfield", "update_Subscribers_format").trim();
		String confirmed_ismandatory = utils.getproperty("mandatoryfield", "update_Subscribers_confirmed").trim();
		String customfields_ismandatory = utils.getproperty("mandatoryfield", "update_Subscribers_customfields").trim();
		String tag_ismandatory = utils.getproperty("mandatoryfield", "update_Subscribers_tag").trim();
		...
		if(items.size() >0) {
			...
			for (Element ele_item : items) {
				...
				if(!isvalid(fieldid, "true", fieldid_format)) {
					errorcount = errorcount + 1; 
					if(errormessage.isEmpty() || errormessage.contains("no access to list")) {
						if(errormessage.contains("no access to list")) {
							errormessage =  utils.getproperty("errormessages", "error_no_list_no_field");
						}
						else {
							errormessage =  utils.getproperty("errormessages", "error_unable_to_load_customfield");
						}
				...
				if(!isvalid(value, "true", value_format)) {
					errorcount = errorcount + 1; 
				}
