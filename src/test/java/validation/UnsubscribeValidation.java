...
public class UnsubscribeValidation extends BaseClass {
	...
	public boolean parameter_Validation(String paraString, HashMap<String, String> error) {
		...
		String emailaddress_ismandatory = utils.getproperty("mandatoryfield", "Unsubscribers_email").trim();
		String mailinglist_ismandatory = utils.getproperty("mandatoryfield", "Unsubscribers_list").trim();
		...
		try {
			...
			emailaddress = ele_emailaddress.getText().toString().trim();

			Element  ele_mailinglist = ele_details.getChild("list");
			mailinglist = ele_mailinglist.getText().toString().trim();
		...
		if(!isvalidlist(mailinglist, mailinglist_ismandatory, allAvailableLists)) {
			errorcount = errorcount + 1; 
			if(errormessage.isEmpty() || errormessage.trim().contains(utils.getproperty("errormessages", "error_missing_email").toLowerCase().trim())) {
				if(emailaddress.trim().isEmpty() || emailaddress.trim()=="") {
					if(mailinglist.isEmpty() || mailinglist.trim()=="") {
						errormessage =  utils.getproperty("errormessages", "error_missing_emailid_listid");
					}
				}
				else {
					if(mailinglist.isEmpty() || mailinglist.trim()=="") {
						...
						errormessage =  utils.getproperty("errormessages", "error_invalid_listid") + " ("+ mailinglist.trim() +")";
					}
				}
