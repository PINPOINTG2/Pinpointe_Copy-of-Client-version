...
import subscribers_common_utilities.RequestDataForCheckUnsubscriber;
import subscribers_common_utilities.ResponseGeneral;
import subscribers_common_utilities.VerifySubscriberExists;
import validation.CheckListforUnsubscribedContactValidation;

public class CheckListforUnsubscribedContact extends BaseClass {

	String methodName = "Check List for UnSubscribed Contact";
	String responseCode = "";
	String responseTime = "";
	String api_version ="";
	String status = "";
	...
	RequestDataForCheckUnsubscriber request_xml = new RequestDataForCheckUnsubscriber();
	...
	CheckListforUnsubscribedContactValidation SubscriberValidation = new CheckListforUnsubscribedContactValidation();

	@Test(testName = "Delete Subscriber", dataProviderClass=TestData.class, dataProvider="getCheckListForUnsubscriber_negData")
	public void DeleteSubscribers(String[] body) {
		...
		if(!api_status) {
			...
			if(!isalreadyExists) {
				...
				String isunsubs = getsubs_response.get("unsubscribeconfirmed1");
				if(isunsubs == null) {
					isunsubs = "";
				}
				if(isunsubs.trim().toLowerCase().equalsIgnoreCase("1") || isunsubs.trim().toLowerCase().equalsIgnoreCase("yes") || isunsubs.trim().toLowerCase().equalsIgnoreCase("true")) {
